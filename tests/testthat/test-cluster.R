# Test hd_run_clustering -------------------------------------------------------
test_that("hd_run_clustering works with various inputs and options", {
  # Example Data
  df <- data.frame(
  ...
  res1 <- hd_run_clustering(hd_object)
  ...
  # 3. Cluster Rows Only
  res3 <- hd_run_clustering(df, cluster_rows = TRUE, cluster_cols = FALSE)
  ...
  res4 <- hd_run_clustering(df, cluster_rows = FALSE, cluster_cols = TRUE)
  ...
  # 5. No Clustering
  res5 <- hd_run_clustering(df, cluster_rows = FALSE, cluster_cols = FALSE)
  ...
  res6 <- hd_run_clustering(df, distance_method = "manhattan", clustering_method = "single")
  ...
  df_missing <- df
  df_missing[1, 2] <- NA
  res7 <- hd_run_clustering(df_missing, distance_method = "euclidean")
