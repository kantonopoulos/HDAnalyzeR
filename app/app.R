      ...
      fluidPage(
        ...
        wellPanel(
          ...
          tags$ul(
            ...
            tags$li("Some steps like classification models may take time — a spinner at the bottom right corner will indicate progress."),
            tags$li(
              "You want to use all functionalities with full flexibility? Check out our package on GitHub: ",
              tags$a(href = "https://github.com/kantonopoulos/HDAnalyzeR", 
                    "HDAnalyzeR", target = "_blank")
            ),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$cols_keep, {
    ...
    # Only keep columns that exist, plus always keep sample_id
    ...
    updated <- data[, unique(c(input$sample_id, valid_cols)), drop = FALSE]
  ...
  observeEvent(input$cols_remove, {
    ...
    valid_cols <- input$columns_to_keep[input$columns_to_keep %in% names(data)]
    # Remove selected columns but never remove sample_id
    updated <- data[, !(names(data) %in% valid_cols) | names(data) == input$sample_id, drop = FALSE]
    processed_data(updated)
  ...
  output$metadata_filter_choices_ui <- renderUI({
    data_to_use <- if (exists("filtered_metadata")) filtered_metadata() else metadata()
    req(data_to_use, input$metadata_filter_column)
    if (input$metadata_filter_column %in% colnames(data_to_use)) {
      choices <- sort(unique(data_to_use[[input$metadata_filter_column]]))
  ...
  observeEvent(input$metadata_keep, {
    req(filtered_metadata(), processed_data(), input$metadata_filter_column, input$metadata_filter_choices, input$sample_id)
    
    meta <- filtered_metadata()
    
    # Filter metadata based on selected categories
    ...
    # Filter processed data based on filtered metadata
  ...
  observeEvent(input$metadata_remove, {
    ...
    # Remove samples from processed data that match filtered metadata
    current_data <- processed_data()
    new_data <- current_data[!(current_data[[input$sample_id]] %in% filtered_ids), ]
    
    processed_data(new_data)
    filtered_metadata(meta[!(meta[[input$sample_id]] %in% filtered_ids), ])  # update filtered metadata
  ...
  observeEvent(input$reset_data, {
    ...
    # Properly copy objects so reactives see a *different* instance
    processed_data(as.data.frame(original_data()))
    filtered_metadata(as.data.frame(metadata()))

    showNotification("Data and metadata have been reset to original.", type = "message")

    # Re-run UI updates that depend on processed_data()
    updateSelectInput(session, "columns_to_keep",
                      choices = setdiff(names(processed_data()), input$sample_id))
  })
  ...
  observeEvent(input$color_variable, {
    ...
    metadata_to_use <- if (exists("filtered_metadata")) filtered_metadata() else metadata()
    merged <- full_join(processed_data(), metadata_to_use, by = input$sample_id)
  ...
  observeEvent(input$run, {
    ...
    tryCatch({
      if (input$method == "PCA") {
        # Check components
        n_features <- ncol(processed_data())
        if (input$components > n_features) {
          showNotification(
            "Number of components exceeds number of features. Please reduce the component number.",
            type = "error"
          )
      ...
      showNotification(
        paste("Error:", e$message, "- Check your data/metadata or component number."),
        type = "error"
      )
      return()  # Skip calculation
    })
  ...
  output$dim_table <- renderDT({
    req(dim_result())
    datatable(dim_result(), options = list(pageLength = 10))
  })
  ...
  observeEvent(c(input$plot_color, input$by_sample), {
    ...
    if (c_type == "continuous" && input$by_sample == TRUE) {
      output$dynamic_pca_palette_ui <- renderUI({
        ...
        data_to_use <- if (exists("filtered_metadata")) filtered_metadata() else metadata()
        unique_categories <- unique(na.omit(data_to_use[[input$plot_color]]))
    
        output$dynamic_pca_palette_ui <- renderUI({
          unique_categories <- unique(data_to_use[[input$plot_color]])
  ...
  observe({
    ...
    if (v_type == "categorical") {
      data_to_use <- if (exists("filtered_metadata")) filtered_metadata() else metadata()
      unique_vals <- sort(unique(na.omit(data_to_use[[input$de_variable]])))
      
      output$de_case_ui <- renderUI({
        tagList(
          # Keep de_case from resetting
  ...
  output$de_table <- renderDT({
    req(de_result())
    datatable(de_result(), options = list(pageLength = 10))
  })
  ...
  observe({
    ...
    if (v_type == "categorical" && isFALSE(input$multiclass)) {
      data_to_use <- if (exists("filtered_metadata")) filtered_metadata() else metadata()
      unique_vals <- sort(unique(na.omit(data_to_use[[input$ml_variable]])))

      output$ml_case_ui <- renderUI({
        tagList(
          # Keep ml_case choices fixed; don't reset selected value
  ...
  output$roc_curve_ml <- renderPlotly({
    ...
    p <- roc_curve() + 
        geom_point(aes(text = paste("1 - specificity:", round(1 - specificity, 2),
                                   "<br>Sensitivity:", round(sensitivity, 2))),
                                  color = NA,
                                  alpha = 0)
  ...
  output$feat_imp_plot_ml <- renderPlotly({
    ...
    p <- feat_imp_plot() + 
        aes(text = paste("Feature:", Feature, "<br>Importance:", round(Scaled_Importance, 2)))
