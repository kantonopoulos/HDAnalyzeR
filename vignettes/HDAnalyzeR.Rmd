...
For whole example workflows you can visit any of the package articles.
...
The final step involves performing an Over-Representation Analysis for the up-regulated proteins from differential expression, that are also identified as features by the ML model. In this example, we will use the Gene Ontology (GO) database and show the results only for AML. We will use `hd_ora()` and `hd_plot_ora()` to perform and visualize the analysis.
