...
`hd_run_qc_summary()` provides a comprehensive summary of the input data and metadata. 

Regarding the data, it will check the column types, calculate the percentage of NAs in each column and row and plot their distributions, calculate protein-protein correlations, and create a heatmap of these correlations. Users can also specify the threshold for reporting protein-protein correlations.

Regarding the metadata, it will check the column types and calculate the percentage of NAs in each column and row exactly as before.  It will also create summary visualizations for the metadata variables like "Sex", "Disease", and "Age".
...
We will run a differential expression analysis to identify potential biomarkers. We will use `hd_run_de_limma()` so that we will be able to correct also for Sex and Age. This method will help us pinpoint proteins that are significantly different between conditions. We will run the analysis 3 times for different case classes against all other classes each time. We will also run `hd_plot_volcano()` to visualize the results. We will present only the results for Acute Myeloid Leukemia (AML).

```{r}
de_res_aml <- hd_run_de_limma(hd_obj,
                              variable = "Disease",
                              case = "AML",
                              correct = c("Sex", "Age"))
de_res_aml <- hd_plot_volcano(de_res_aml, logfc_lim = 1)
...
de_res_crc <- hd_run_de_limma(hd_obj,
                              variable = "Disease",
                              case = "CRC",
                              correct = c("Sex", "Age"))
```
...
```{r, results = 'hide', message = FALSE, warning = FALSE}
de_res_lungc <- hd_run_de_limma(hd_obj,
                                variable = "Disease",
                                case = "LUNGC",
                                correct = c("Sex", "Age"))
```
...
In addition to differential expression analysis, we will use a Lasso machine learning classification model to identify significant features. This model will help us understand which proteins are most predictive of the conditions being studied. 

We will start by creating the data split with `hd_run_data_split()` so we don't have any data leakage between train and test data and then we will run the model with `hd_run_rreg()`. Once again, we will run the analysis 3 times but we will present only the results for AML.

```{r, warning = FALSE, message = FALSE}
hd_split <- hd_run_data_split(hd_obj, variable = "Disease")

lasso_res_aml <- hd_run_rreg(hd_split, 
                             variable = "Disease",
                             case = "AML",
                             mixture = 1,
                             grid_size = 5,
...
lasso_res_aml$var_imp_plot
```

```{r, results = 'hide', message = FALSE, warning = FALSE}
lasso_res_crc <- hd_run_rreg(hd_split, 
                             variable = "Disease",
                             case = "CRC",
                             mixture = 1,
                             grid_size = 5,
...
```{r, results = 'hide', message = FALSE, warning = FALSE}
lasso_res_lungc <- hd_run_rreg(hd_split, 
                               variable = "Disease",
                               case = "LUNGC",
                               mixture = 1,
                               grid_size = 5,
...
The final step involves performing an Over-Representation Analysis for the up-regulated proteins from differential expression, that are also identified as features by the ML model. In this example, we will use the Gene Ontology (GO) database and show the results only for AML. We will use `hd_run_ora()` and `hd_plot_ora()` to perform and visualize the analysis.
...
enrichment <- hd_run_ora(intersect_proteins, 
                         database = "GO", 
                         ontology = "BP", 
                         pval_lim_enrichment = 0.2)  # We will use a higher p-value threshold for this example
