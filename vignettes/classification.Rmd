...
> 📓 In the whole vignette the `verbose` parameter of the model functions will be set to FALSE in order to keep this guide clean and concise. However, in a real case we recommend to leave it to default (TRUE) in order to know the model's progress.
...
> 📓 We should not forget to update the `plot_title` argument by changing the metrics from "accuracy", "sensitivity", "apwcificity", and "auc" to "rmse" and "rsq".
...
Furthermore, we can validate our trained model in new data. For this example we will not use another dataset, but we will split the data initially to create a train and a validation set and then split the validation set to a train and a test set. We will use this second split to initially train the model and then evaluate it with the validation data. In a real case scenario, you can do either this, or use a completely different dataset to check that our model generalizes properly. We will use the `hd_model_test()` function to do this. Let's see an example with the AML model.
