...
If our data have a single predictor, we can use `hd_run_lr()` instead of `hd_run_rreg()` to perform a logistic regression. Random forest can be used as it was for multiple predictors.
...
split_obj <- hd_run_data_split(hd_obj_single, variable = "Disease", ratio = 0.8)

model_res <- hd_run_lr(split_obj, case = "AML", palette = "cancers12", verbose = FALSE)
...
split_obj <- hd_run_data_split(hd_obj, variable = "Disease", ratio = 0.8)

model_res <- hd_run_rreg(split_obj, case = "AML", cv_sets = 3, grid_size = 5, verbose = FALSE)
...
```{r, message = FALSE, warning = FALSE}
model_aml <- hd_run_rreg(split_obj, case = "AML", cv_sets = 3, grid_size = 5, verbose = FALSE)

model_gliom <- hd_run_rreg(split_obj, case = "GLIOM", cv_sets = 3, grid_size = 5, verbose = FALSE)

split_obj_sex <- hd_run_data_split(hd_obj |> hd_filter_by_sex(variable = "Sex", sex = "F"), 
                                   variable = "Disease", 
                                   ratio = 0.8)

model_ovc <- hd_run_rreg(split_obj_sex, case = "OVC", cv_sets = 3, grid_size = 5, verbose = FALSE)
```
...
```{r, message = FALSE, warning = FALSE}
model_cll <- hd_run_rreg(split_obj, case = "AML", control = "CLL", cv_sets = 3, grid_size = 5, verbose = FALSE)

model_blood <- hd_run_rreg(split_obj, 
                           case = "AML", 
...
model_all <- hd_run_rreg(split_obj, case = "AML", cv_sets = 3, grid_size = 5, verbose = FALSE)

de_cll <- hd_run_de_limma(hd_obj, case = "AML", control = "CLL", correct = c("Sex", "Age"))

de_blood <- hd_run_de_limma(hd_obj, 
                            case = "AML", 
                            control = c("CLL", "MYEL", "LYMPH"), 
                            correct = c("Sex", "Age"))

de_all <- hd_run_de_limma(hd_obj, case = "AML", correct = c("Sex", "Age"))
```
