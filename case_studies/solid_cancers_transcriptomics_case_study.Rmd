...
```{r}
# Kidney
pca_variance_kidney <- pca_kidney |>   
  hd_plot_pca_loadings(displayed_pcs = 1, displayed_features = 10)

# Lung
pca_variance_lung <- pca_lung |>   
  hd_plot_pca_loadings(displayed_pcs = 1, displayed_features = 10)

# Endometrial
pca_variance_endometrial <- pca_endometrial |>   
  hd_plot_pca_loadings(displayed_pcs = 1, displayed_features = 10)

combined_variance_plots <- pca_variance_kidney$pca_loadings_plot |
...
```{r}
# Kidney
top_features_kidney <- pca_variance_kidney$pca_loadings |> 
  filter(component == "PC1") |> 
  arrange(-abs(value)) |> 
  head(2) |> 
  pull(terms)

top_features_kidney_plot <- hd_plot_feature_boxplot(hd_kidney, 
...
# Lung
top_features_lung <- pca_variance_lung$pca_loadings |> 
  filter(component == "PC1") |> 
  arrange(-abs(value)) |> 
  head(2) |> 
  pull(terms)

top_features_lung_plot <- hd_plot_feature_boxplot(hd_lung, 
...
# Endometrial
top_features_endometrial <- pca_variance_endometrial$pca_loadings |> 
  filter(component == "PC1") |> 
  arrange(-abs(value)) |> 
  head(2) |> 
  pull(terms)

top_features_endometrial_plot <- hd_plot_feature_boxplot(hd_endometrial, 
...
```{r}
volcano_kidney <- de_kidney |> 
  hd_plot_volcano(report_nproteins = FALSE,
                  top_up_prot = 3,
                  top_down_prot = 3)

volcano_lung <- de_lung |> 
  hd_plot_volcano(report_nproteins = FALSE,
                  top_up_prot = 3,
                  top_down_prot = 3)


volcano_endometrial <- de_endometrial |> 
  hd_plot_volcano(report_nproteins = FALSE,
                  top_up_prot = 3,
                  top_down_prot = 3)


combined_volcano_plots <- volcano_kidney$volcano_plot | 
...
```{r}
# Kidney
top_features_de_kidney <- de_kidney$de_res |> 
  slice_max(logFC, n = 1) |>         
...
# Lung
top_features_de_lung <- de_lung$de_res |> 
  slice_max(logFC, n = 1) |>         
...
# Endometrial
top_features_de_endometrial <- de_endometrial$de_res |> 
  slice_max(logFC, n = 1) |>         
...
combined_de_boxplots <- top_features_de_kidney_plot | 
  top_features_de_lung_plot |
  top_features_de_endometrial_plot
